...
SAFETY CONTRACT (enforced in code below):
  * This client issues **GET requests only**. The single low-level HTTP helper,
    ``_get``, hard-codes the HTTP method to GET. There is no POST/PUT/DELETE path,
    so the client is structurally incapable of mutating Meta Ads.
  * It requests only the ``ads_read`` scope worth of data. No ``ads_management``
    endpoints are ever called.
  * Calls are rate-limited (client-side throttle) and retried with exponential
    backoff on transient failures.

It pulls campaigns, adsets, and ads with their insights, then normalizes each
entity into a flat snapshot dict the rest of the system understands.
"""
...
log = get_logger("meta_client")

# Fields requested for each object level.
...
# Insight metrics we care about. ``actions`` / ``action_values`` carry results,
...
# Action types that count as a "result" depending on the campaign objective.
...
class MetaApiError(Exception):
    """Raised for non-retryable Meta API errors (auth, permissions, bad request)."""


class MetaRateLimitError(Exception):
    """Raised when Meta signals rate limiting; safe to retry with backoff."""
...
class MetaClient:
    ...
    def _throttle(self) -> None:
        """Simple client-side rate limiter: enforce a minimum gap between calls."""
        with self._lock:
    ...
    def _get(self, path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Perform a single rate-limited, retried GET request.

        This is the ONLY method that talks to the network, and it only ever
        issues GET requests - the read-only guarantee lives here.
        """
        params = dict(params or {})
        url = path if path.startswith("http") else f"{self._base_url}/{path.lstrip('/')}"
        # Absolute pagination 'next' URLs already carry a signed access_token and
        # cursor; only inject the token for our own relative paths to avoid a
        # duplicate access_token query parameter.
        ...
        def _do_request() -> Dict[str, Any]:
            ...
            # Try to extract Meta's structured error.
            try:
            ...
            # Rate limiting: codes 4, 17, 32, 613 or HTTP 429 -> retry.
            if resp.status_code == 429 or code in {4, 17, 32, 613}:
                log.warning("Meta rate limit hit (code=%s): %s", code, message)
                raise MetaRateLimitError(message)

            # Everything else (auth/permission/bad request) is non-retryable.
    ...
    def _get_all_pages(
        ...
        *,
        ...
        """Follow cursor pagination and return all ``data`` rows."""
        ...
        while next_url and pages < max_pages:
            payload = self._get(next_url)  # next_url is absolute and pre-signed
            results.extend(payload.get("data", []))
            next_url = payload.get("paging", {}).get("next")
            pages += 1
        return results
    ...
    def verify_token(self) -> Dict[str, Any]:
        """Lightweight call to confirm the token works. Returns the /me payload."""
    ...
    def fetch_account_entities(
        ...
        *,
        ...
        """Fetch campaigns, adsets and ads for an account, each with insights.

        Returns a flat list of normalized snapshot dicts (one per entity level).
        ``date_preset`` controls the insights window (e.g. 'today', 'yesterday',
        'last_7d'). Per-entity insights are requested at each level so we capture
        rejected/limited delivery states even when an ad has no spend.
        """
        ...
        # 4) Insights per level, indexed by object id for fast joining.
        ...
        # 5) Build normalized snapshots.
        for camp in campaigns:
            entities.append(
                self._build_snapshot(
                    level="campaign",
                    obj=camp,
                    insight=campaign_insights.get(camp["id"], {}),
                    campaign=camp,
                )
            )
        ...
        for adset in adsets:
            ...
            entities.append(
                self._build_snapshot(
                    level="adset",
                    obj=adset,
                    insight=adset_insights.get(adset["id"], {}),
                    campaign=camp,
                    adset=adset,
                )
            )
        ...
        log.info(
            "Fetched %d entities for %s (%d campaigns, %d adsets, %d ads)",
            len(entities), account, len(campaigns), len(adsets), len(ads),
        )
        return entities

    def _fetch_insights_map(
        self, account: str, level: str, date_preset: str
    ) -> Dict[str, Dict[str, Any]]:
        """Fetch insights at a given level and index them by object id."""
    ...
    def _sum_actions(actions: Optional[List[Dict[str, Any]]], wanted: set) -> float:
        """Sum the 'value' of actions whose action_type is in ``wanted``."""
        if not actions:
            return 0.0
        total = 0.0
        for a in actions:
    ...
    def _build_snapshot(
        ...
        *,
        ...
        """Flatten a Meta object + its insights into a normalized snapshot dict."""
        ...
        # Ad review status, when present on an ad object.
        review_status = None
        feedback = ad.get("ad_review_feedback")
        if isinstance(feedback, dict):
            # Surface the first disapproval reason if any.
